...
export async function createPlan(baseUrl, request) {
  const response = await fetch(`${trimBaseUrl(baseUrl)}/qgis/plans`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ request }),
  });
  await ensureOk(response, "Plan creation failed");
  return response.json();
}

export async function getPlan(baseUrl, planId) {
  const response = await fetch(`${trimBaseUrl(baseUrl)}/qgis/plans/${encodeURIComponent(planId)}`);
  await ensureOk(response, "Plan fetch failed");
  return response.json();
}

export async function listPlans(baseUrl, { sessionId = "", status = "active", limit = 20 } = {}) {
  const params = new URLSearchParams({
    session_id: sessionId || "",
    status: status || "active",
    limit: String(Math.min(Math.max(1, Number(limit) || 20), 200)),
  });
  const response = await fetch(`${trimBaseUrl(baseUrl)}/qgis/plans?${params}`);
  await ensureOk(response, "Plan list failed");
  const data = await response.json();
  return data.plans || [];
}

export async function patchPlan(baseUrl, planId, patch) {
  const response = await fetch(`${trimBaseUrl(baseUrl)}/qgis/plans/${encodeURIComponent(planId)}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(patch || {}),
  });
  await ensureOk(response, "Plan update failed");
  return response.json();
}

export async function approvePlan(baseUrl, planId) {
  const response = await fetch(`${trimBaseUrl(baseUrl)}/qgis/plans/${encodeURIComponent(planId)}/approve`, {
    method: "POST",
  });
  await ensureOk(response, "Plan approval failed");
  return response.json();
}

export async function rejectPlan(baseUrl, planId) {
  const response = await fetch(`${trimBaseUrl(baseUrl)}/qgis/plans/${encodeURIComponent(planId)}/reject`, {
    method: "POST",
  });
  await ensureOk(response, "Plan rejection failed");
  return response.json();
}

export async function runPlan(baseUrl, planId, request = null) {
  const response = await fetch(`${trimBaseUrl(baseUrl)}/qgis/plans/${encodeURIComponent(planId)}/run`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(request ? { request } : {}),
  });
  await ensureOk(response, "Plan run failed");
  return response.json();
}
